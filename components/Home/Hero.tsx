  ...
  return (
    <header className="flex h-screen py-12 space-y-8 lg:py-0 lg:space-y-0 lg:h-[calc(100vh-6rem)] items-center mt-[6rem]  flex-col lg:flex-row overflow-hidden bg-primary-grey">
      <div className="px-8 " data-aos="fade-right">
        <motion.h1 className="text-3xl block md:text-4xl mb-8 py-2 lg:text-5xl leading-12  text-primary-black">
          Find Your Dream <br />{" "}
          <span className="text-primary-green">Home</span> Today
        </motion.h1>

        <motion.p className="w-[80%] lg:w-[60%] text-slate-700 leading-6 border-l-primary-black border-solid border-l  pl-8 opacity-70 mb-10  ">
        ...
        <motion.div className="flex items-center gap-[10%]">
          ...
          >
            <svg
              ...
              xmlSpace="preserve"
              className="shadow-[0_0_20px_#00cc66] rounded-full"
            >
              <g>
                <g>
                  <path
          ...
          {imgArr.map((img, index) => {
            return (
              <img
                src={img}
                key={`${img}-${index}`}
                className={`h-[50px] md:h-[80px] transition-all ease-out cursor-pointer hover:scale-110  duration-300 w-[50px] md:w-[80px] ${
